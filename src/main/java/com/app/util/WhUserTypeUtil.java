...
public class WhUserTypeUtil {
	public void generatePieChart(String location,List<Object[]> list) {
		...
		JFreeChart chart = ChartFactory.createPieChart("WHUSER TYPE CODE", dataset);
	...
	public void generateBarChart(String location,List<Object[]> list) {
		...
		// b. Convert DataSet data into JFreeChart object using ChartFactory class
		JFreeChart chart = ChartFactory.createBarChart("WHUSER TYPE CODE", "USER TYPES", "COUNT", dataset);
		
		// c. Convert JFreeChart object into one Image using ChartUtil class
		try {
			ChartUtils.saveChartAsJPEG(new File(location+"/whusertypeB.jpg"), chart, 500, 350);
		} catch (IOException e) {
			e.printStackTrace();
		}
	}	
