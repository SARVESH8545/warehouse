...
public class SaleOrderUtil {
	public void generatePieChart(String location,List<Object[]> list) {
		...
		JFreeChart chart = ChartFactory.createPieChart("SALEORDER STOCK SOURCE", dataset);
	...
	public void generateBarChart(String location,List<Object[]> list) {
		...
		// b. Convert DataSet data into JFreeChart object using ChartFactory class
		JFreeChart chart = ChartFactory.createBarChart("SALEORDER STOCK SOURCE", "STOCK SOURCE", "COUNT", dataset);

		// c. Convert JFreeChart object into one Image using ChartUtil class
		try {
			ChartUtils.saveChartAsJPEG(new File(location+"/saleOrderB.jpg"), chart, 500, 350);
		} catch (IOException e) {
			e.printStackTrace();
		}
	}
